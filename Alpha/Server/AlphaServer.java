...
public class AlphaServer {
    ...
    void removeResponsor(HostMessage hostMessage){
        connections.remove(hostMessage);
    }
    ...
    /**
     * 设置服务器响应超时事件,单位为毫秒
     * @param timeout 时间,单位为毫秒
     * @throws AlphaServerException     如果发生异常{@link SocketException},则抛出打包异常{@link AlphaServerException}
     */
    ...
    protected HostMessage checkConnected(Socket connection) throws AlphaServerException {
        HostMessage client = new HostMessage(connection);
     ...
     * 返回当前服务端中所有客户信息
     * <p>
     *     客户信息的遍历请使用Foreach,或者迭代器方法
     * @return 服务端所有客户信息
     */
    public HostMessage[] getAllClientMessage(){
        HostMessage[] HostMessages = new HostMessage[connections.size()];
        AlphaResponsorSet.AlphaHashKeySetIterator iterator = connections.keySetIterator();
        int i = 0;
        for(iterator.first(); !iterator.isDone(); iterator.next(), i++){
            HostMessages[i] = iterator.currrentInstance();
        }
        return HostMessages;
    }
    ...
    public Responsor getResponsor(HostMessage client){
    ...
    // event
    private void connectionIn(HostMessage newClient, AlphaInputStream acceptor, AlphaOutputStream sender){
